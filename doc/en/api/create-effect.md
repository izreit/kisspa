...
|`owner`|`object` (optional)|if provided, GC can auto-cancel the effect. (experimental)|
...
When any of those properties change, `fun` is scheduled to run again.
...
If passed, the effect is cancelled when `owner` is collected by GC.
